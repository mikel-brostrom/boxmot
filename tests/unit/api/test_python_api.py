...
from boxmot.engine.config import BOXMOT_DEFAULTS, DEFAULT_DETECTOR, DEFAULT_REID, get_mode_default
...
def test_package_root_lazily_reexports_python_api():
    ...
    assert importlib.import_module("boxmot.api.pipeline").BoxMOT is api_module.BoxMOT
...
def test_boxmot_eval_namespace_uses_shared_reid_default_when_reid_is_none(tmp_path):
    ...
    args = model._base_eval_args(experiment="mot17-mini-train-yolox-lmbn")
...
def test_boxmot_eval_namespace_treats_inherited_defaults_as_non_explicit():
    ...
    args = model._base_eval_args(experiment="mot17-mini")
...
def test_boxmot_tune_namespace_accepts_explicit_split():
    ...
    assert args.experiment == "mot17-mini-train-yolox-lmbn"
    assert args.benchmark == ""
    assert args.split == "ablation"
    assert args.split_explicit is True
...
def test_boxmot_eval_namespace_uses_explicit_tracker_backend():
    ...
    args = model._base_eval_args(experiment="mot17-mini", tracker_backend="cpp")
...
def test_public_reid_reuses_preselected_torch_device(monkeypatch):
    reid_module = importlib.import_module("boxmot.reid.core.runtime")
...
def test_tune_result_str_shows_delta_vs_baseline(monkeypatch):
    ...
    baseline_trial = api_results_module.TuneTrialResult(
        index=1, config={"track_buffer": 30}, metrics=baseline_metrics, score=(66.0,)
    )
    best_trial = api_results_module.TuneTrialResult(
        index=2, config={"track_buffer": 40}, metrics=best_metrics, score=(67.5,)
    )
    tune = api_results_module.TuneResult(
        benchmark="mot17-mini",
        tracker="bytetrack",
...
def test_boxmot_val_tune_and_export_facades(monkeypatch, tmp_path):
    ...
    model = api_module.BoxMOT(
        detector="yolov8n", reid="lmbn_n_duke", tracker="boosttrack", classes=[0, 1], project=tmp_path / "runs"
    )

    metrics = model.val(experiment="mot17-mini", device="cpu")

    assert metrics.summary["HOTA"] == 50.0
    eval_args, eval_config, eval_kwargs = calls["eval"]
    assert eval_args.experiment == "mot17-mini"
    ...
    tune_results = model.tune(experiment="mot17-mini", n_trials=3, device="cpu")
    ...
    assert tune_args.experiment == "mot17-mini"
    ...
    export_results = model.export(
        include=("onnx",),
        device="cpu",
        coreml_batch_buckets=(1, 8, 32),
        coreml_minimum_deployment_target="macOS15",
        coreml_compute_units="CPUOnly",
        coreml_timeout=120,
        coreml_max_memory_gb=8,
    ...
    assert export_args.coreml_batch_buckets == (1, 8, 32)
    assert export_args.coreml_minimum_deployment_target == "macOS15"
    assert export_args.coreml_compute_units == "CPUOnly"
    assert export_args.coreml_timeout == 120.0
    assert export_args.coreml_max_memory_gb == 8.0
...
def test_boxmot_train_and_eval_reid_facades(monkeypatch, tmp_path):
    ...
    assert train_args.head_type == "multiscale_channel2"
    assert train_args.multiscale_channel_alpha == 0.4
...
def test_boxmot_val_logs_cli_like_intro_without_printing_report(monkeypatch, tmp_path, capsys):
    ...
    assert ("__panel__:Dataset", [("Experiment", "mot17-mini")]) in workflow.fields
...
def test_boxmot_generate_and_research_facades(monkeypatch, tmp_path):
    ...
    generated = model.generate(experiment="mot17-mini", device="cpu", batch_size=8, resume=False)
    ...
    assert generate_args.experiment == "mot17-mini"
    ...
    assert research_args.experiment == "mot17-mini"
...
def test_boxmot_generate_requires_exactly_one_input(tmp_path):
    ...
    with pytest.raises(ValueError, match="exactly one of experiment=... or source=..."):
        model.generate()

    with pytest.raises(ValueError, match="exactly one of experiment=... or source=..."):
        model.generate(experiment="mot17-mini", source=tmp_path / "dataset")
