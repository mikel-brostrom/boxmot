...
from tests.performance.reid.benchmark_inference import (
    _coreml_is_current as benchmark_coreml_is_current,
)
from tests.performance.reid.benchmark_inference import (
    _onnx_is_current as benchmark_onnx_is_current,
)
...
def test_benchmark_coreml_cache_rejects_same_mtime_source_replacement(tmp_path):
    ...
    weights.write_bytes(b"source-one")
    os.utime(weights, (1, 1))
    model = ModelSpec("test", weights)
    model.coreml.mkdir()
    write_artifact_metadata(
        model.coreml,
        {
            "format": "coreml_mlprogram",
            "source_sha256": file_sha256(weights),
            "batch_buckets": [1, 8],
            "input_shape": [3, 384, 128],
        },
    )
    args = types.SimpleNamespace(
        coreml_batch_buckets="1,8",
        imgsz=(384, 128),
    )

    assert benchmark_coreml_is_current(model, args)

    weights.write_bytes(b"source-two")
    os.utime(weights, (1, 1))

    assert not benchmark_coreml_is_current(model, args)
...
def test_onnx_export_dynamic_uses_dynamic_shapes(monkeypatch, tmp_path, batch_size):
    ...
    def fake_export(model, args, f, dynamo=None, **kwargs):
        calls.append((args, {**kwargs, "dynamo": dynamo}))
        Path(f).touch()
...
def test_onnx_export_quiet_mode_uses_dynamo_when_available(monkeypatch, tmp_path):
    ...
    out_file = tmp_path / "stripe_pool.pt"
    ...
    assert exporter.export() == out_file.with_suffix(".onnx")
    assert len(calls) == 1
    assert calls[0][1]["dynamo"] is True
    assert "dynamic_axes" not in calls[0][1]
    assert "dynamic_shapes" not in calls[0][1]


@pytest.mark.parametrize("has_external_initializer", [False, True])
def test_onnx_export_records_actual_opset_and_cleans_only_unused_sidecar(
    monkeypatch,
    tmp_path,
    ...
    fake_onnx_model = types.SimpleNamespace(
        ir_version=9,
        opset_import=[types.SimpleNamespace(domain="", version=18)],
        graph=types.SimpleNamespace(initializer=[initializer]),
    )
    fake_onnx = types.SimpleNamespace(
        __version__="0.0-test",
        checker=types.SimpleNamespace(check_model=lambda _model: None),
        load=lambda _path, **_kwargs: fake_onnx_model,
    ...
    def fake_export(model, args, f, dynamo=None, **kwargs):
        del model, args, dynamo, kwargs
        Path(f).touch()
        sidecars["exact"] = Path(f"{f}.data")
        sidecars["neighbor"] = Path(f"{f}.data.keep")
        sidecars["exact"].write_bytes(b"external")
        sidecars["neighbor"].write_bytes(b"keep")
    ...
    metadata = onnx_exporter_module.read_artifact_metadata(exported)

    assert metadata["opset"] == 18
    assert metadata["export_contract"]["requested_opset"] == 17
    assert sidecars["exact"].exists() is has_external_initializer
    assert sidecars["neighbor"].is_file()
...
def test_ensure_onnx_export_reuses_current_file(monkeypatch, tmp_path):
    ...
    contract = onnx_exporter_module._onnx_export_contract(
        object(),
        opset=None,
        dynamic=False,
        half=False,
    ...
    write_artifact_metadata(
        ...
        {
            "dynamic_batch": False,
            "export_fingerprint": export_content_fingerprint(weights, contract),
            "artifact_sha256": file_sha256(onnx_file),
        },
    )
...
def test_ensure_onnx_export_refreshes_when_source_content_changes(monkeypatch, tmp_path):
    weights = tmp_path / "model.pt"
    onnx_file = tmp_path / "model.onnx"
    ...
    original_mtime = weights.stat().st_mtime
    weights.write_bytes(b"other")
    os.utime(weights, (original_mtime, original_mtime))
    calls = []
...
def test_benchmark_onnx_freshness_rejects_tampered_same_mtime_artifact(tmp_path):
    weights = tmp_path / "model.pt"
    onnx_file = tmp_path / "model.onnx"
    weights.write_bytes(b"weights")
    onnx_file.write_bytes(b"onnx")
    expected_shape = (2, 3, 384, 128)
    contract = onnx_exporter_module._onnx_export_contract(
        types.SimpleNamespace(shape=expected_shape),
        opset=18,
        dynamic=False,
        half=False,
        simplify=False,
    ...
    model = ModelSpec("model", weights)

    assert benchmark_onnx_is_current(model, expected_shape, opset=18)

    original_mtime = onnx_file.stat().st_mtime
    onnx_file.write_bytes(b"evil")
    os.utime(onnx_file, (original_mtime, original_mtime))

    assert not benchmark_onnx_is_current(model, expected_shape, opset=18)
