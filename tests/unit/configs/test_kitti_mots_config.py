...
@pytest.mark.parametrize(
    "split_changes", [{"modalities": {"ground_truth": None, "ground_truth_3d": None}}, {"has_ground_truth": False}]
)
...
def _write_experiment(tmp_path: Path) -> Path:
    ...
    path.write_text(
        yaml.safe_dump(
            {
                "dataset": {"ref": "kitti-mots", "split": "train", "modalities": {"images": {}, "ground_truth": {}}},
...
def test_kitti_mots_experiment_preserves_annotations_selection_and_native_class_map(tmp_path: Path) -> None:
    ...
    assert dataset_modalities(config["dataset"], "train")["ground_truth"]["paths"] == [
