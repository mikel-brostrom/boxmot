...
from boxmot.trackers.common.motion.cmc.ecc import ECC
...
def test_ecc_nonconvergence_returns_fresh_identity(monkeypatch):
    ecc = ECC(scale=1.0)
    image = np.zeros((20, 20, 3), dtype=np.uint8)
    ecc.apply(image)

    def fail_after_mutating_warp(*args, **kwargs):
        del kwargs
        warp = args[2]
        warp[0, 2] = 1234.0
        error = cv2.error("ECC did not converge")
        error.code = cv2.Error.StsNoConv
        raise error

    monkeypatch.setattr(cv2, "findTransformECC", fail_after_mutating_warp)

    result = ecc.apply(image)

    np.testing.assert_array_equal(result, np.eye(2, 3, dtype=np.float32))


def test_ecc_rejects_low_correlation_transform(monkeypatch):
    ecc = ECC(scale=1.0, min_correlation=0.5)
    image = np.zeros((20, 20, 3), dtype=np.uint8)
    ecc.apply(image)
    bad_warp = np.array([[1.0, 0.0, 2.0], [0.0, 1.0, -12.0]], dtype=np.float32)

    monkeypatch.setattr(
        cv2,
        "findTransformECC",
        lambda *args, **kwargs: (0.2, bad_warp.copy()),
    )
...
def test_restore_transform_scale_conjugates_homography():
    ecc = ECC(warp_mode=cv2.MOTION_HOMOGRAPHY, scale=1.0)
    ecc._preprocess_scale = (0.5, 0.25)
    scaled = np.array(
        [
            [1.1, 0.2, 5.0],
            [0.3, 0.9, -4.0],
    ...
    restored = ecc.restore_transform_scale(scaled)
    ...
    np.testing.assert_allclose(restored, expected, rtol=1e-6, atol=1e-7)


def test_restore_transform_scale_handles_upscaling():
    ecc = ECC(scale=2.0)
    ecc._preprocess_scale = (2.0, 2.0)
    scaled = np.array([[1.0, 0.0, 6.0], [0.0, 1.0, -4.0]], dtype=np.float32)

    restored = ecc.restore_transform_scale(scaled)

    expected = np.array([[1.0, 0.0, 3.0], [0.0, 1.0, -2.0]], dtype=np.float32)
    np.testing.assert_allclose(restored, expected)


def test_sof_rejects_nonfinite_affine(monkeypatch):
    sof = SOF(scale=1.0, min_inliers=4, min_inlier_ratio=0.5)
    image = np.zeros((20, 20, 3), dtype=np.uint8)
    keypoints = np.array(
        [[[1.0, 1.0]], [[1.0, 5.0]], [[5.0, 1.0]], [[5.0, 5.0]]],
        dtype=np.float32,
    )
    sof.prev_frame = sof.preprocess(image)
    sof.prev_keypoints = keypoints.copy()
    sof.initialized = True

    monkeypatch.setattr(
        cv2,
        "calcOpticalFlowPyrLK",
        lambda *args, **kwargs: (keypoints.copy(), np.ones((4, 1), dtype=np.uint8), None),
    )
    monkeypatch.setattr(
        cv2,
        "estimateAffinePartial2D",
        lambda *args, **kwargs: (
            np.array([[1.0, 0.0, np.nan], [0.0, 1.0, 0.0]], dtype=np.float32),
            np.ones((4, 1), dtype=np.uint8),
        ),
    )
    monkeypatch.setattr(cv2, "goodFeaturesToTrack", lambda *args, **kwargs: keypoints.copy())

    result = sof.apply(image)
