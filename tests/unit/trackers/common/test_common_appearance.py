...
def _batch(embs: np.ndarray | None = None) -> _DetectionBatch:
    dets = np.array([[1, 2, 11, 22, 0.9, 3]], dtype=np.float32)
    return _DetectionBatch.from_layout(dets, AABB_DETECTIONS, embs=embs)
...
def test_resolve_batch_embeddings_reuses_precomputed_embeddings():
    ...
    resolved = resolve_batch_embeddings(_batch(embs))
...
def test_resolve_batch_embeddings_preserves_precomputed_empty_dimension():
    batch = _batch(np.ones((1, 3), dtype=np.float32)).select([])

    resolved = resolve_batch_embeddings(batch)

    assert resolved.shape == (0, 3)


def test_resolve_batch_embeddings_never_runs_a_model_and_requires_supplied_features():
    with np.testing.assert_raises_regex(ValueError, "required when use_embeddings=True"):
        resolve_batch_embeddings(_batch())
