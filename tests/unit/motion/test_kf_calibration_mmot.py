...
from boxmot.motion.kalman_filters import calibration
...
def test_mmot_corner_conversion_preserves_metadata_and_uses_enclosing_aabb() -> None:
    ...
    converted = calibration._obb_to_cxywh(raw)
...
def test_load_gt_data_reads_external_mmot_annotations(tmp_path: Path) -> None:
    ...
    loaded = calibration.load_gt_data(sequence_root / "sequence", annotations_root)
...
def test_mmot_tracks_feed_the_current_four_dimensional_kf_calibration(tmp_path: Path) -> None:
    ...
    tracks, widths, heights = calibration.build_tracks_from_sequence(
...
def test_process_noise_is_zero_for_constant_velocity_tracks() -> None:
    ...
    q_position, q_velocity = calibration._estimate_process_noise([(measurements, states, 0)])
...
def test_kf_parameterization_metadata_remains_explicit() -> None:
    assert calibration.tracker_kf_type("botsort") == "xywh"
    assert calibration.tracker_kf_type("sfsort") is None
    assert calibration._measurement_labels("xywh") == ["cx", "cy", "w", "h"]
    assert calibration._get_dim_x("xysr") == 7
    assert calibration._get_dim_z("xysr") == 4


def test_kf_calibration_has_no_positional_detection_cache_path() -> None:
    parameters = inspect.signature(calibration.estimate_kf_noise).parameters

    assert "dets_root" not in parameters
    assert "iou_threshold" not in parameters
    assert not hasattr(calibration, "estimate_R_from_detections")
    assert not hasattr(calibration, "run_kf_tuning")
