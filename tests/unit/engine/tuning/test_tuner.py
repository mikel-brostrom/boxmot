...
def test_write_trial_yaml_emits_reusable_scalar_config(tmp_path):
    yaml_cfg = {
        "parent": {
            "type": "choice",
            "options": [False, True],
    ...
    write_trial_yaml(
        ...
        output_path,
        base_config={"parent": False, "child": True, "leaf": 0.5, "plain": 1.0},
    )
...
def test_built_in_tracker_yaml_combines_runtime_defaults_and_tuning_metadata():
    yaml_cfg = load_yaml_config("bytetrack")
    runtime_defaults = load_tracker_defaults("bytetrack")
    flat_config = flatten_yaml_config(yaml_cfg)
...
def test_build_tune_workflow_fields_uses_experiment_selector() -> None:
    args = SimpleNamespace(
        tracker="bytetrack",
    ...
    assert fields["Experiment"] == "mot17-mini"
