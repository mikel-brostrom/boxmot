...
def test_resolve_experiment_runtime_normalizes_models_to_absolute_paths(monkeypatch, tmp_path):
    model_dir = tmp_path / "models"
    model_dir.mkdir()
    source_dir = tmp_path / "data"
    source_dir.mkdir()

    def fake_apply_evaluation_config(probe):
        probe.source = source_dir
        probe.experiment_id = "mot17-mini"
        probe.dataset_id = "mot17"
        probe.benchmark = "mot17"
        return {"benchmark": {}}

    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(research_module, "apply_evaluation_config", fake_apply_evaluation_config)
    monkeypatch.setattr(
    ...
    source_root, experiment_id, dataset_id, benchmark, detector_path, reid_path, cfg = (
        research_module._resolve_experiment_runtime("mot17-mini")
    )
...
def test_build_eval_payload_uses_shared_cache_project():
    ...
    researcher.dataset_id = "mot17"
    researcher.benchmark = "mot17"
    researcher.evaluation_cfg = {"benchmark": {}}
    ...
    assert payload["experiment_id"] == "mot17-mini"
    assert payload["dataset_id"] == "mot17"
    assert payload["benchmark"] == "mot17"
