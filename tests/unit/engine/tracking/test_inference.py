...
def test_sanitize_detections_filters_nonfinite_and_unbounded_obb_with_aligned_masks():
    dets = np.array(
        [
            [32, 32, 20, 10, 0.2, 0.9, 0],
            [32, 32, np.nan, 10, 0.2, 0.9, 0],
            [32, 32, np.inf, 10, 0.2, 0.9, 0],
            [32, 32, 500, 10, 0.2, 0.9, 0],
            [1e20, 32, 20, 10, 0.2, 0.9, 0],
        ],
        dtype=np.float32,
    )
    masks = np.arange(5 * 8 * 8, dtype=np.uint8).reshape(5, 8, 8)

    filtered, filtered_masks, keep = sanitize_detections(
        dets,
        masks,
        image_shape=_DUMMY_IMG.shape,
    )

    np.testing.assert_array_equal(keep, np.array([True, False, False, False, False]))
    np.testing.assert_array_equal(filtered, dets[:1])
    np.testing.assert_array_equal(filtered_masks, masks[:1])
...
def test_model_config_detector_defaults_match_separator_variants():
    ...
    assert detector_cfg["id"] == "yolo11l-mmot-obb"
    assert default_imgsz("yolo11l_3ch.pt") == detector_cfg["imgsz"]
...
def test_configure_benchmark_runtime_lets_model_config_override_dataset_detector(monkeypatch):
    ...
    monkeypatch.setattr(evaluator_module, "_load_evaluation_cfg", lambda _args: benchmark_bundle)
...
def test_configure_benchmark_runtime_uses_explicit_component_configs_without_benchmark(monkeypatch):
    ...
    monkeypatch.setattr(evaluator_module, "_load_evaluation_cfg", lambda _args: {})
...
def test_apply_class_remap_skips_obb_without_loading_aabb_mapping(monkeypatch):
    ...
    args = SimpleNamespace(
        ...
        experiment="/tmp/local-mmot.yaml",
    )
...
def test_run_motmetrics_only_expands_sequences_for_single_class(
    ...
    monkeypatch.setattr(evaluator_module, "_load_evaluation_cfg", lambda args: {"benchmark": {}})
...
def test_build_eval_workflow_fields_reports_effective_cpp_backend(tmp_path):
    ...
    # Dataset card
    assert ("Experiment", "mot17-mini") in fields["__panel__:Dataset"]
...
def test_run_eval_refreshes_workflow_fields_after_setup(monkeypatch, tmp_path):
    ...
    assert dataset_items["Experiment"] == "mot17-mini"
