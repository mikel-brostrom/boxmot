...
def test_eval_dispatch_preserves_display_flags(monkeypatch, flags, tuning) -> None:
    captured = {}
    ...
    assert captured["args"].kf_tuning is tuning


def test_eval_help_describes_final_replay_visualization() -> None:
    result = CliRunner().invoke(boxmot, ["eval", "--help"])

    assert result.exit_code == 0, result.output
    assert "--show" in result.output
    assert "--save" in result.output
    assert "final selected replay" in " ".join(result.output.split())
