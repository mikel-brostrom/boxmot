...
default, so the timing isolates tracker work from detector and encoder inference.
...
NATIVE_TRACKERS = supported_native_trackers()
...
def _make_rectangular_masks(
    detections: FloatArray,
    image_hw: tuple[int, int],
) -> MaskBatch:
    """Create full-frame foreground masks aligned to synthetic boxes."""

    height, width = image_hw
    values = torch.zeros((len(detections), height, width), dtype=torch.bool)
    for index, (x1, y1, x2, y2, _score, _class_id) in enumerate(detections):
        left = max(0, min(int(x1), width - 1))
        top = max(0, min(int(y1), height - 1))
        right = max(left + 1, min(int(np.ceil(x2)), width))
        bottom = max(top + 1, min(int(np.ceil(y2)), height))
        values[index, top:bottom, left:right] = True
    return MaskBatch(values.contiguous())
...
def _build_workload(
    *,
    ...
    masks = _make_rectangular_masks(base_detections, image_hw) if with_masks else None
    frames: list[FrameInput] = []
    for frame_index, (rows, embeddings) in enumerate(zip(detection_frames, embedding_frames)):
        sample_id = f"benchmark:{frame_index}"
        frame = Frame(
            image=image_tensor,
            sample_id=sample_id,
        ...
        frames.append(FrameInput(frame=frame, detections=detections))
    return tuple(frames)
...
def _build_tracker(
    ...
    """Construct a decoupled tracker from an immutable canonical spec."""

    return create_tracker(TrackerSpec(name=tracker_name, backend=backend, geometry="aabb"))
...
def _measure(
    ...
    *,
    ...
    tracker = _build_tracker(tracker_name, backend)
    requirements = tracker.requirements
    frames = _build_workload(
        detection_count=detection_count,
        frame_count=warmup_count + measured_count,
        image_hw=image_hw,
        seed=seed,
        with_embeddings=requirements.embeddings,
        with_masks=requirements.masks,
    )
    warmup_frames = frames[:warmup_count]
    measured_frames = frames[warmup_count:]

    for item in warmup_frames:
        tracker.update(item.detections, item.frame if requirements.frame else None)

    started = time.perf_counter()
    for item in measured_frames:
        tracker.update(item.detections, item.frame if requirements.frame else None)
    ...
    return {
        ...
        "backend": backend,
        "requires_embeddings": requirements.embeddings,
        "requires_masks": requirements.masks,
        "requires_frame": requirements.frame,
        "embedding_dim": DEFAULT_EMBEDDING_DIM if requirements.embeddings else "",
...
def _print_table(rows: list[ResultRow]) -> None:
    ...
    table = Table(title="BoxMOT tracker FPS (synthetic detections; structured v24 inputs)")
    ...
    for row in rows:
        ...
        required_inputs = [
            label
            for key, label in (
                ("requires_embeddings", "embeddings"),
                ("requires_masks", "masks"),
                ("requires_frame", "frame"),
            )
        ...
        table.add_row(
            str(row["tracker"]),
            str(row["backend"]),
            ", ".join(required_inputs) or "detections",
            str(row["n_dets"]),
            str(row["frames"]),
