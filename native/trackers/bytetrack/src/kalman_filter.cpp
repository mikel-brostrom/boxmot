...
KalmanFilterXYAH::KalmanFilterXYAH() {
    ...
    update_mat_.setZero();
    update_mat_.block<4, 4>(0, 0).setIdentity();
}

KalmanFilterXYAH::Vector KalmanFilterXYAH::InitialCovarianceStd(const Eigen::Vector4d& measurement) const {
    Vector std(8);
    std << 2.0 * std_weight_position_ * measurement[3],
        2.0 * std_weight_position_ * measurement[3],
        1.0e-2,
        2.0 * std_weight_position_ * measurement[3],
        10.0 * std_weight_velocity_ * measurement[3],
        10.0 * std_weight_velocity_ * measurement[3],
        1.0e-5,
        10.0 * std_weight_velocity_ * measurement[3];
    return std;
...
std::pair<KalmanFilterXYAH::Vector, KalmanFilterXYAH::Vector> KalmanFilterXYAH::ProcessNoiseStd(
    const Vector& mean
) const {
    Vector std_pos(4);
    Vector std_vel(4);
...
KalmanFilterXYAH::Vector KalmanFilterXYAH::MeasurementNoiseStd(const Vector& mean, const float confidence) const {
    const double weight = 1.0 - static_cast<double>(confidence);
    Vector std(4);
    std << weight * std_weight_position_ * mean[3],
        weight * std_weight_position_ * mean[3],
        weight * 1.0e-1,
        weight * std_weight_position_ * mean[3];
    return std;
}

std::pair<KalmanFilterXYAH::Vector, KalmanFilterXYAH::Matrix> KalmanFilterXYAH::Initiate(
...
std::pair<KalmanFilterXYAH::Vector, KalmanFilterXYAH::Matrix> KalmanFilterXYAH::Predict(
    const Vector& mean,
...
std::pair<KalmanFilterXYAH::Vector, KalmanFilterXYAH::Matrix> KalmanFilterXYAH::Project(
    const Vector& mean,
    const Matrix& covariance,
...
std::pair<KalmanFilterXYAH::Vector, KalmanFilterXYAH::Matrix> KalmanFilterXYAH::Update(
    const Vector& mean,
    const Matrix& covariance,
    const Eigen::Vector4d& measurement,
    const float confidence
    ...
    const Matrix projected_cov_inv = llt.solve(Matrix::Identity(4, 4));
    const Matrix kalman_gain = covariance * update_mat_.transpose() * projected_cov_inv;
    const Vector innovation = measurement - projected_mean;
    ...
    updated_mean[2] = std::max(updated_mean[2], 1.0e-4);
    updated_mean[3] = std::max(updated_mean[3], 1.0e-4);
...
std::pair<KalmanFilterXYWH::Vector, KalmanFilterXYWH::Matrix> KalmanFilterXYWH::Initiate(const Vector& measurement) const {
