...
Eigen::Vector4d ObbToXyxy(const Eigen::Matrix<double, 5, 1>& box) {
    const cv::RotatedRect rect(
        ...
        static_cast<float>(box[4] * 180.0 / 3.14159265358979323846)
    );
...
std::vector<Eigen::VectorXf> OnnxReIdModel::GetFeatures(const std::vector<Detection>& detections, const cv::Mat& image) const {
...
cv::Mat OnnxReIdModel::ExtractCrop(const Detection& detection, const cv::Mat& image) const {
    ...
    const Eigen::Vector4d xyxy = detection.is_obb ? ObbToXyxy(detection.xywha) : detection.xyxy;
    const cv::Rect rect = ClampRect(xyxy, image.size());
...
std::optional<OnnxReIdModel> MaybeCreateOnnxReIdModel(const fs::path& model_path, const std::string& preprocess_name) {
    if (model_path.empty()) {
        return std::nullopt;
    }
