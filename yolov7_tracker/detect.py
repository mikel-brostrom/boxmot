...
def count_object(
    center, id, middle, left, right, l_count, r_count, left_list, right_list,
    ):
    
    ix, iy = center

    if (ix > left) and (ix < middle):
        if id not in left_list:
            left_list.append(id)

    elif (ix < right) and (ix > middle):
        if id not in right_list:
    ...
    return l_count, r_count, left_list, right_list
