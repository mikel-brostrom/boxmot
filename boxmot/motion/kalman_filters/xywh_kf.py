...
class KalmanFilterXYWH(BaseKalmanFilter):
    ...
    def _get_initial_covariance_std(self, measurement: np.ndarray) -> np.ndarray:
        return [
            2 * self._std_weight_position * measurement[2],  # 2 x 0.05 x w
            2 * self._std_weight_position * measurement[3],  # 2 x 0.05 x h
    ...
    def _get_process_noise_std(self, mean: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        std_pos = [
            self._std_weight_position * mean[2],  # 0.05 x w
            self._std_weight_position * mean[3],  # 0.05 x h
            self._std_weight_position * mean[2],
            self._std_weight_position * mean[3]
        ]
        std_vel = [
            self._std_weight_velocity * mean[2],  # 0.00625 x w
            self._std_weight_velocity * mean[3],  # 0.00625 x h
