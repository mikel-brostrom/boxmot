"""Immutable detector construction specifications."""
...
def _validate_json_value(value: JSONValue, *, location: str) -> None:
    """Reject mutable or non-JSON option values from a frozen specification."""
...
def _validate_common_spec(
    *,
    ...
    if artifact is not None and (not isinstance(artifact, str) or not artifact or artifact != artifact.strip()):
...
class DetectorSpec:
    """Everything needed to construct and fingerprint a detector runtime."""
...
__all__ = ("DetectorSpec", "JSONScalar", "JSONValue")
