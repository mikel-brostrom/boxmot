...
"""High-level BoxMOT Python API facade."""
...
def _validate_generate_inputs(*, experiment: str | Path | None, source: str | Path | None) -> None:
    has_experiment = experiment is not None and str(experiment) != ""
    has_source = source is not None and str(source) != ""
    if has_experiment == has_source:
        raise ValueError("Provide exactly one of experiment=... or source=... when calling BoxMOT.generate().")
...
def _validate_eval_inputs(
    *,
    ...
    has_experiment = experiment is not None and str(experiment) != ""
    has_dataset = dataset is not None and str(dataset) != ""
    if has_experiment == has_dataset:
        raise ValueError("Provide exactly one of experiment=... or dataset=... when calling BoxMOT.val().")
...
def _resolve_train_model_spec(value: Any) -> tuple[str, str] | None:
    ...
    from boxmot.engine.config import list_training_recipes
    ...
    model_names = {name.lower(): name for name in registered_backbone_names()}
...
class BoxMOT:
    ...
    def generate(
        ...
        *,
        ...
        _validate_generate_inputs(experiment=experiment, source=source)
        ...
        return GenerateResult(
            benchmark=(
                str(getattr(args, "benchmark", None) or getattr(args, "experiment", None) or "") or None
            ),
    ...
    def val(
        ...
        *,
        ...
        _validate_eval_inputs(experiment=experiment, dataset=dataset)
    ...
    def train(
        ...
        *,
        ...
        multiscale_channel_alpha: float = BOXMOT_DEFAULTS.train.multiscale_channel_alpha,
        ...
        anatomical_token_dim: int = BOXMOT_DEFAULTS.train.anatomical_token_dim,
        ...
        anatomical_auxiliary: bool = BOXMOT_DEFAULTS.train.anatomical_auxiliary,
        anatomical_metadata_dir: str | Path | None = BOXMOT_DEFAULTS.train.anatomical_metadata_dir,
        anatomical_min_keypoint_confidence: float = BOXMOT_DEFAULTS.train.anatomical_min_keypoint_confidence,
        anatomical_distill_weight: float = BOXMOT_DEFAULTS.train.anatomical_distill_weight,
        anatomical_attention_weight: float = BOXMOT_DEFAULTS.train.anatomical_attention_weight,
        anatomical_visibility_weight: float = BOXMOT_DEFAULTS.train.anatomical_visibility_weight,
        anatomical_contrastive_weight: float = BOXMOT_DEFAULTS.train.anatomical_contrastive_weight,
        anatomical_pose_teacher_weight: float = BOXMOT_DEFAULTS.train.anatomical_pose_teacher_weight,
        anatomical_target_type: str = BOXMOT_DEFAULTS.train.anatomical_target_type,
        anatomical_teacher_momentum: float = BOXMOT_DEFAULTS.train.anatomical_teacher_momentum,
        anatomical_multiscale: bool = BOXMOT_DEFAULTS.train.anatomical_multiscale,
        anatomical_deployment: bool = BOXMOT_DEFAULTS.train.anatomical_deployment,
        anatomical_deployment_dim: int = BOXMOT_DEFAULTS.train.anatomical_deployment_dim,
        anatomical_deployment_alpha: float = BOXMOT_DEFAULTS.train.anatomical_deployment_alpha,
        anatomical_deployment_id_weight: float = BOXMOT_DEFAULTS.train.anatomical_deployment_id_weight,
        anatomical_deployment_metric_weight: float = BOXMOT_DEFAULTS.train.anatomical_deployment_metric_weight,
        anatomical_temperature: float = BOXMOT_DEFAULTS.train.anatomical_temperature,
        ...
        payload = {
            ...
            "head_type": head_type,
            "multiscale_channel_alpha": float(multiscale_channel_alpha),
            ...
            "anatomical_token_dim": int(anatomical_token_dim),
            ...
            "anatomical_auxiliary": bool(anatomical_auxiliary),
            "anatomical_metadata_dir": (
                None
                if anatomical_metadata_dir is None
                else str(anatomical_metadata_dir)
            ),
            "anatomical_min_keypoint_confidence": float(
                anatomical_min_keypoint_confidence
            ),
            "anatomical_distill_weight": float(anatomical_distill_weight),
            "anatomical_attention_weight": float(anatomical_attention_weight),
            "anatomical_visibility_weight": float(anatomical_visibility_weight),
            "anatomical_contrastive_weight": float(
                anatomical_contrastive_weight
            ),
            "anatomical_pose_teacher_weight": float(
                anatomical_pose_teacher_weight
            ),
            "anatomical_target_type": str(anatomical_target_type),
            "anatomical_teacher_momentum": float(
                anatomical_teacher_momentum
            ),
            "anatomical_multiscale": bool(anatomical_multiscale),
            "anatomical_deployment": bool(anatomical_deployment),
            ...
            "anatomical_deployment_id_weight": float(
                anatomical_deployment_id_weight
            ),
            "anatomical_deployment_metric_weight": float(
                anatomical_deployment_metric_weight
            ),
            "anatomical_temperature": float(anatomical_temperature),
