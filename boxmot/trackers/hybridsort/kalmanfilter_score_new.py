...
class KalmanFilterNew_score_new(object):
    ...
    **Example**
    ...
    def __init__(self, dim_x: int, dim_z: int, dim_u: int = 0, args: Optional[Any] = None):
        if dim_x < 1:
    ...
    def update(
        ...
        R: Optional[Union[np.ndarray, float]] = None,
    ...
    def predict_steadystate(self, u: Union[int, np.ndarray] = 0, B: Optional[np.ndarray] = None):
        """
    ...
    def update_steadystate(self, z: Optional[np.ndarray]):
        """
    ...
    def batch_filter(
        self,
        zs: List,
        Fs: Optional[List] = None,
        Qs: Optional[List] = None,
        Hs: Optional[List] = None,
        Rs: Optional[List] = None,
        Bs: Optional[List] = None,
        us: Optional[List] = None,
    ...
    def log_likelihood_of(self, z: Optional[np.ndarray]):
        """
...
def predict(
    x: np.ndarray,
    P: np.ndarray,
    F: Union[np.ndarray, int] = 1,
    Q: Union[np.ndarray, int] = 0,
    u: Union[np.ndarray, int] = 0,
    B: Union[np.ndarray, int] = 1,
    alpha: float = 1.0,
):
    """
    Predict next state (prior) using the Kalman filter state propagation
...
def rts_smoother(Xs: np.ndarray, Ps: np.ndarray, Fs: List, Qs: List):
    """
    Runs the Rauch-Tung-Striebel Kalman smoother on a set of
    means and covariances computed by a Kalman filter. The usual input
    would come from the output of `KalmanFilter.batch_filter()`.
