...
from boxmot.appearance.reid_model_factory import ModelRegistry
...
class BaseModelBackend:
    def __init__(self, weights, device, half):
        ...
        self.model_name = ModelRegistry.get_model_name(self.weights)

        self.model = ModelRegistry.build_model(
            self.model_name,
            num_classes=ModelRegistry.get_nr_classes(self.weights),
    ...
    def download_model(self, w):
        if w.suffix == ".pt":
            model_url = ModelRegistry.get_model_url(w)
            if not w.exists() and model_url is not None:
                ...
                ModelRegistry.show_downloadable_models()
                exit()