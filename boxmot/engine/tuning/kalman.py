...
from boxmot.motion.kalman_filters.fitting import MIN_COVARIANCE_SCALE, ProcessNoiseMoments, ScalarNoiseMoments
...
class KalmanCalibrationResult:
    ...
    def record_tuning(self, fixed_options: Mapping[str, Any]) -> None:
        """Record which calibrated settings must remain fixed when tuning resumes."""
        report = json.loads(self.report_path.read_text(encoding="utf-8"))
...
def calibrate_kalman(
    ...
    *,
    ...
    from boxmot.engine.tuning.kalman_data import load_calibration_data
    from boxmot.engine.tuning.kalman_model import CalibrationModel, validate_calibration_options
