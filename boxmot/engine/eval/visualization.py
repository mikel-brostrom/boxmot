...
class ReplayVisualization:
    ...
    Videos use a 30 FPS output grid and hold the previous rendered observation
    across capture gaps. Repeated output images never become tracker updates.
    Sequences without timestamps use one output frame per observation.
    """
    ...
    def _write_until(self, end_time: float) -> None:
        ...
        while self._next_tick < end_tick:
            self._writer.write(self._previous_image)
            self._next_tick += 1
    ...
    def _open_video(self, image: np.ndarray) -> None:
        ...
        writer = cv2.VideoWriter(str(path), cv2.VideoWriter_fourcc(*"mp4v"), self.video_fps, (width, height))
    ...
    def __call__(self, replayed: ReplayFrame) -> None:
        """Consume exactly one tracked observation without changing its time."""
        if self._closed:
        ...
        image = render_result(sample.frame, replayed.result, class_names=self.class_names)
