...
:class:`~boxmot.engine.ui.core.ui.WorkflowProgress` with a clean, index-based
...
class PipelineTracker:
    ...
    def show_interactive(self) -> None:
        ...
        """
        from boxmot.engine.ui.workflow.interactive import interactive_step_viewer
    ...
    def __exit__(
        ...
        if exc_val is not None:
            # The complete error panel is printed below after the live region
            # has been discarded.  Painting it here may make a normal-screen
            # Live taller than the terminal and commit its prefix to
            # scrollback before Rich can erase it.
            self._workflow.fail(error=exc_val, render=False)
        ...
        want_interactive = exc_val is None and self._interactive_on_exit and self._step_records

        # Always discard the live frame and cleanly reprint one final panel.
        # Rich refreshes a normal-screen Live once from ``Live.stop()``.  If
        # the pending final state is installed first and is taller than the
        # terminal, its prefix scrolls out of the erasable viewport and is
        # left above the static final panel.  Keep the last bounded progress
        # frame installed while stopping instead.
        self._workflow.transient = True
        if self._workflow._live is not None:
            self._workflow._live.transient = True

        self._workflow.stop(refresh_final=False)
        if self._wire_status_fns:
...
def _set_status_fns(callback: WorkflowDetailCallback | None) -> None:
    ...
    from boxmot.resources.download import set_download_status_fn
