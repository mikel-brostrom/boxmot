...
from boxmot.engine.ui.workflow.fields import WORKFLOW_PANEL_PREFIX
...
class WorkflowProgress:
    ...
    def stop(self, *, refresh_final: bool = True) -> None:
        """Stop the live display.

        Parameters
        ----------
        refresh_final:
            Update the active :class:`~rich.live.Live` region with the latest
            workflow state before stopping it.  Callers that render their own
            static final panel should pass ``False`` so a newly-expanded final
            state is never painted into the transient live region first.
        """
        if not self._started:
            return

        used_alt_screen = self._uses_alt_screen
        final_renderable = self._live_renderable() if refresh_final else None
        if self._live is not None:
            if refresh_final and self._last_rendered_state != self._state_snapshot():
                assert final_renderable is not None
        ...
        if used_alt_screen and not self.transient and final_renderable is not None:
