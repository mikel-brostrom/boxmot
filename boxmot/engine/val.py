...
from boxmot.engine.detectors import (
    default_imgsz,
    get_yolo_inferer,
...
from boxmot.utils import DATASET_CONFIGS, NUM_THREADS, TRACKER_CONFIGS, TRACKEVAL, WEIGHTS
...
def eval_init(
    args,
    trackeval_dest=TRACKEVAL,  # type: Path
...
def generate_dets_embs(args, y, source):  # type: (argparse.Namespace, Path, Path) -> None
    """
    ...
    for r in args.reid_model:
        ...
        embs_path = (
            args.project / "dets_n_embs" / y.stem / "embs" / r.stem / (source.parent.name + ".txt")
        )
...
def parse_mot_results(results):  # type: (str) -> Dict[str, Union[float, int]]
    """
    ...
    for section, fields_map in metric_specs.values():
        match = re.search(rf"{re.escape(section)}.*?COMBINED\s+(.*?)\n", results, re.DOTALL)
        if match:
...
def trackeval(args, seq_paths, save_dir, gt_folder, metrics=None):  # type: (argparse.Namespace, List[Path], Path, Path, Optional[List[str]]) -> str
    """
    ...
    args_list = [
        ...
        *metrics,
        ...
        "True",
        "--GT_LOC_FORMAT",
        "{gt_folder}/{seq}/gt/gt_temp.txt",
        "--SEQ_INFO",
        *d,
    ]

    p = subprocess.Popen(args=args_list, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def process_single_det_emb(y, source_path, opt):  # type: (Path, Path, argparse.Namespace) -> None
    new_opt = copy.deepcopy(opt)
    generate_dets_embs(new_opt, y, source=source_path / "img1")


def run_generate_dets_embs(opt):  # type: (argparse.Namespace) -> None
        ...
        for i, mot_folder_path in enumerate(mot_folder_paths):
            ...
            if dets_path.exists() and embs_path.exists():
                if not prompt_overwrite("Detections and Embeddings", dets_path, opt.ci):
                    LOGGER.debug(
                        f"Skipping generation for {mot_folder_path} as they already exist."
                    )
                    continue
            tasks.append((y, mot_folder_path))

        LOGGER.info(
            f"Generating detections and embeddings for {len(tasks)} sequences with model {y.name}"
        )

        with concurrent.futures.ProcessPoolExecutor(max_workers=NUM_THREADS) as executor:
            futures = [
                executor.submit(process_single_det_emb, y, source_path, opt)
...
def process_sequence(
    ...
    model_name,  # type: str
    reid_name,  # type: str
    tracking_method,  # type: str
    exp_folder,  # type: str
    target_fps,  # type: Optional[int]
    device,  # type: str
    cfg_dict=None,  # type: Optional[Dict]
):  # type: (...) -> tuple
...
def _worker_init():  # type: () -> None
    # each spawned process needs its own sinks
    _configure_logging()


def run_generate_mot_results(opt, evolve_config=None):  # type: (argparse.Namespace, Optional[dict]) -> None
    # Prepare experiment folder
    base = (
        opt.project
        / "mot"
        / f"{opt.yolo_model[0].stem}_{opt.reid_model[0].stem}_{opt.tracking_method}"
    )
    ...
    sequence_names = sorted(
        [d.name for d in Path(opt.source).iterdir() if d.is_dir() and (d / "img1").exists()]
    )
    ...
    seq_frame_nums = {}  # type: Dict[str, List[int]]

    with concurrent.futures.ProcessPoolExecutor(
        max_workers=NUM_THREADS, initializer=_worker_init
    ) as executor:
...
def run_trackeval(opt):  # type: (argparse.Namespace) -> Dict[str, Union[float, int]]
    """
...
def load_dataset_cfg(name):  # type: (str) -> Dict
