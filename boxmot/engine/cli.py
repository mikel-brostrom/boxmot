...
def core_options(func):
    options = [
        ...
        click.option(
            "--project", type=Path, default=ROOT / "runs", help="save results to project/name"
        ),
        ...
        click.option("--gsi", is_flag=True, help="apply Gaussian smoothing interpolation"),
        click.option("--show", is_flag=True, help="display tracking in a window"),
        click.option(
            "--show-labels/--hide-labels", default=True, help="show or hide detection labels"
        ),
        click.option(
            "--show-conf/--hide-conf", default=True, help="show or hide detection confidences"
        ),
...
def singular_model_options(func):
    options = [
        ...
        click.option("--classes", type=int, multiple=True, help="filter by class indices"),
    ]
    for opt in reversed(options):
        func = opt(func)
    return func
...
def plural_model_options(func):
    options = [
        ...
        click.option(
            "--classes", type=int, multiple=True, default=[0], help="filter by class indices"
        ),
    ]
    for opt in reversed(options):
        func = opt(func)
...
def export_options(func):
    ...
    options = [
        ...
        click.option(
            "--optimize", is_flag=True, help="Optimize TorchScript for mobile (CPU export only)"
        ),
        click.option(
            "--dynamic", is_flag=True, help="Enable dynamic axes for ONNX/TF/TensorRT export"
        ),
        click.option("--simplify", is_flag=True, help="Simplify ONNX model"),
        click.option("--opset", type=int, default=12, help="ONNX opset version"),
...
def tune_options(func):
    ...
    options = [
        click.option(
            "--n-trials", type=int, default=4, help="number of trials for evolutionary tuning"
        ),
        click.option(
            "--objectives",
            type=str,
