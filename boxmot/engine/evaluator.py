...
from boxmot.detectors import (default_imgsz, get_yolo_inferer, is_rtdetr_model,
...
def generate_dets_embs(args: argparse.Namespace, y: Path, source: Path) -> None:
    ...
    for r in args.reid_model:
        reid_model = ReidAutoBackend(weights=r, device=yolo.predictor.device, half=args.half).model
    ...
    for frame_idx, r in enumerate(tqdm(results, desc="Frames")):
        ...
        for reid, reid_model_name in zip(reids, args.reid_model):
            ...
            embs_path = (
                args.project / "dets_n_embs" / y.stem / "embs" / reid_model_name.stem / (source.parent.name + ".txt")
            )
...
def run_generate_dets_embs(opt: argparse.Namespace) -> None:
    ...
    for y in opt.yolo_model:
        ...
        if len(tasks) == 0:
            ...
            LOGGER.info(
                f"Generating detections and embeddings for {len(tasks)}/{total_sequences} sequences with model {y.name}"
            )
...
def run_generate_mot_results(opt: argparse.Namespace, evolve_config: dict = None) -> None:
    ...
    sequence_names = sorted([d.name for d in Path(opt.source).iterdir() if d.is_dir() and (d / "img1").exists()])
