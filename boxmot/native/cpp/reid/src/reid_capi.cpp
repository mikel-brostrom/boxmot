...
//
// Exposes ``OnnxReIdModel`` to host languages, including the low-level Python
// ctypes binding used by the native appearance encoder.
...
using ::boxmot::reid::OnnxReIdModel;
using ::boxmot::reid::ReIdBackend;
using ::boxmot::reid::ReIdDevice;
...
cv::Mat WrapImage(const std::uint8_t* image_data, int image_rows, int image_cols, int image_channels) {
    ...
    const int cv_type = ::boxmot::native::CvImageType(
...
cv::Rect BoxFromXyxy(const float* row, const cv::Size& image_size) {
    ...
    return ::boxmot::reid::ClampBoxToImage(xyxy, image_size);
}
