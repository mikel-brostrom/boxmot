...
ReIdBackend ResolveBackend(ReIdBackend requested) {
    ...
    const std::optional<std::string> env = GetEnvironmentValue("BOXMOT_REID_BACKEND");
    if (!env.has_value() || *env == "auto") {
        return ReIdBackend::kAuto;
    }
    if (*env == "opencv") {
    ...
    throw std::invalid_argument(
        "Invalid BOXMOT_REID_BACKEND value '" + *env +
        "'. Expected one of: auto, onnxruntime, opencv.");
}
...
ReIdDevice ResolveDevice(ReIdDevice requested) {
    ...
    const std::optional<std::string> env = GetEnvironmentValue("BOXMOT_REID_DEVICE");
    if (!env.has_value() || *env == "auto") return ReIdDevice::kAuto;
    if (*env == "cpu") return ReIdDevice::kCpu;
    if (*env == "cuda") return ReIdDevice::kCuda;
    if (*env == "coreml") return ReIdDevice::kCoreMl;
    throw std::invalid_argument(
        "Invalid BOXMOT_REID_DEVICE value '" + *env +
        "'. Expected one of: auto, cpu, cuda, coreml.");
}
