"""Reusable workflow for offline HP-GRD privileged-teacher caches.

This standalone entrypoint consumes precomputed tensors only; it never
imports or downloads a teacher model.  Example::

    uv run --no-sync python -m boxmot.engine.commands.reid.privileged_cache build \
...
def build_privileged_cache(
    *,
    ...
    reserved_extra = {
        "builder": "boxmot.reid.training.privileged_cache",
...
__all__ = (
    ...
    "load_dataset_index",
    "load_precomputed_teacher_signal_bundle",
    "load_precomputed_teacher_tensors",
