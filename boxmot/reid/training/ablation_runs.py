"""Validated, fingerprinted run helpers for ReID ablations.

The shell scripts remain convenient experiment manifests, while this module is
the single source of truth for architecture validation and safe
complete/resume/fresh run discovery.
"""
...
def build_ablation_spec(config: ReIDTrainConfig) -> AblationSpec:
    """Build a validated, fingerprinted ablation specification."""

    if config.run.resume:
        raise ValueError("A planned ablation specification must not include resume state")
