...
class LMBN_ain_n(nn.Module):
    def __init__(self, args = None, test_only: bool = True, num_classes: int = None, loss = None, pretrained = None, use_gpu = None):
        super(LMBN_ain_n, self).__init__()

        num_classes = args.num_classes if num_classes is None else num_classes
        test_only = args.test_only if args is not None else test_only
        feats = args.feats if args is not None else 512
        activation_map = args.activation_map if args is not None else False
    ...
    def forward(self, x):
        ...
        if not self.training:
            ...
            features = torch.stack([f_glo[0], f_glo_drop[0], f_p0[0], f_p1[0], f_p2[0], f_c0[0], f_c1[0]], dim=2)
