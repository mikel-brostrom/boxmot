...
class TimingStats:
    ...
    def print_summary(self):
        ...
        for key in ["preprocess", "inference", "postprocess"]:
            ...
            avg = total / frames
            log(
                f"{key.capitalize():<20} | <blue>{total:<12.1f}</blue> | <blue>{avg:<12.2f}</blue> | {pct(total):<12.1f}"
            )

        det_avg = det_total / frames
        log(
            f"<bold>{'Detection (total)':<20}</bold> | <cyan>{det_total:<12.1f}</cyan> | <cyan>{det_avg:<12.2f}</cyan> | {pct(det_total):<12.1f}"
        )
        ...
        log(
            f"{'ReID':<20} | <blue>{reid_total:<12.1f}</blue> | <blue>{reid_avg:<12.2f}</blue> | {pct(reid_total):<12.1f}"
        )

        assoc_avg = assoc_time / frames
        log(
            f"{'Association':<20} | <blue>{assoc_time:<12.1f}</blue> | <blue>{assoc_avg:<12.2f}</blue> | {pct(assoc_time):<12.1f}"
        )

        track_total = self.totals["track"]
        track_avg = track_total / frames
        log(
            f"<bold>{'Track (total)':<20}</bold> | <cyan>{track_total:<12.1f}</cyan> | <cyan>{track_avg:<12.2f}</cyan> | {pct(track_total):<12.1f}"
        )
        ...
        log(
            f"{'Plotting':<20} | <blue>{plot_time:<12.1f}</blue> | <blue>{plot_avg:<12.2f}</blue> | {pct(plot_time):<12.1f}"
        )

        overhead_avg = overhead / frames
        log(
            f"{'Other (I/O, etc)':<20} | <blue>{overhead:<12.1f}</blue> | <blue>{overhead_avg:<12.2f}</blue> | {pct(overhead):<12.1f}"
        )
        ...
        log(
            f"<bold>{'Total':<20}</bold> | <cyan>{total_time:<12.1f}</cyan> | <cyan>{avg_total:<12.2f}</cyan> | {sum_pct:<12.1f}"
        )
