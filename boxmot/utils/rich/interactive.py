...
if TYPE_CHECKING:
    from boxmot.utils.rich.pipeline import StepRecord
...
def _scroll_renderable(
    ...
    buf = StringIO()
    cons = _RichConsole(
        file=buf, force_terminal=True, width=max(20, width),
        color_system="truecolor", theme=ui._boxmot_theme(),
    )
    ...
    if active_step is not None and active_step == 0:
        ...
        if detail_parts:
            ...
            if max_detail_height > 0:
                inner, total_lines, clamped_offset = _scroll_renderable(
                    inner, _detail_width(), max_detail_height, scroll_offset,
                )
        ...
        if renderable is not None or text:
            content: RenderableType = (
                renderable if renderable is not None
                else ui._decode_ansi(text or "")
            )
            if max_detail_height > 0:
                content, total_lines, clamped_offset = _scroll_renderable(
                    content, _detail_width(), max_detail_height, scroll_offset,
                )
...
def interactive_step_viewer(
    ...
    panel, total_lines, scroll_offset = _build_full_interactive_panel(
        labels, records, workflow, cursor, active_step,
        scroll_offset=scroll_offset, max_detail_height=_max_detail_height(),
    )
