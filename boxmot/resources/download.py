...
def set_download_status_fn(status_fn: Any) -> None:
    ...
    A renderer may optionally expose ``bar``, ``parallel_bars``, and
    ``tqdm_proxy`` methods. Pass ``None`` to clear the registration.
    """
...
def download_file(
    ...
    *,
    ...
    When ``status_fn`` exposes ``.bar()``, progress is delegated to that
    renderer instead of producing a separate tqdm display.
    """
    dest = Path(dest)
...
def download_files_parallel(
    ...
    *,
    ...
    When a status callback exposing ``parallel_bars`` is registered via
    :func:`set_download_status_fn`, all pending downloads share one progress
    surface. Otherwise this falls back to sequential :func:`download_file`
    calls so cached files cost nothing.
    """
...
def extract_zip(
    ...
    *,
    ...
    When ``status_fn`` exposes ``.bar()``, extraction progress is delegated
    to that renderer instead of producing a separate tqdm display.
    """
...
def download_hf_dataset(repo_id: str, dest: Path, overwrite: bool = False, status_fn: Any = None) -> None:
    ...
    if _has_workflow_bar(status_fn):
        # Delegate byte progress to the caller-provided renderer.
        _ctx = status_fn.bar(
        ...
        class _WorkflowTqdm(base_tqdm):
            ...
            _FLUSH_BYTES = 1 << 20   # flush to the renderer every 1 MiB
            _FLUSH_SECS = 0.08       # … or every 80 ms
...
def snapshot_download_hf_subfolder(
    ...
    *,
    ...
    def _make_tqdm_aggregator(progress_tqdm: type) -> type:
        shared_fetch_task = [None]

        class _TqdmAggregated(progress_tqdm):
    ...
    if status_fn is not None and callable(getattr(status_fn, "tqdm_proxy", None)):
        with status_fn.tqdm_proxy(progress_description, unit="files") as progress_tqdm:
            snapshot_download(tqdm_class=_make_tqdm_aggregator(progress_tqdm), **snapshot_kwargs)
        return

    snapshot_download(tqdm_class=_make_tqdm_aggregator(tqdm), **snapshot_kwargs)
