...
if str(EXAMPLES_ROOT) not in sys.path:
    sys.path.append(str(EXAMPLES_ROOT))  # add strong_sort ROOT to PATH
...
class Evaluator:
    ...
    def run(self, opt):
        ...
        """
        e = Evaluator(opt)
        val_tools_path = EXAMPLES_ROOT / 'val_utils'
...
def parse_opt():
    ...
    parser.add_argument('--project', default=EXAMPLES_ROOT / 'runs' / 'val', help='save results to project/name')
...
if __name__ == "__main__":
    opt = parse_opt()
    check_requirements(requirements=EXAMPLES_ROOT / 'requirements.txt', exclude=('tensorboard', 'thop'))
    e = Evaluator(opt)
    e.run(opt)
