...
from YOLOv6.yolov6.data.datasets import LoadData
...
sys.path.append(r'E:\datasets\strongsort_yolov5\Yolov5_StrongSORT_OSNet\YOLOv6')
import torch
...
def run(
        source='0',
        yolov6_weights=WEIGHTS / 'yolov6.pt',
        yolov6_yaml=ROOT / 'yolov6.yaml',
        yolov6_cfg=ROOT / 'configs/yolov6.yml',
    ...
    # yolov6_yaml = r'E:\datasets\strongsort_yolov5\Yolov5_StrongSORT_OSNet\YOLOv6\data\custom_data.yam;'
    # source = r'E:\DJI_SPARK_LUMS_02-08-2022\DJI_2640.MP4'  # str(source)
    ...
    # Directories)
    if not isinstance(yolov6_weights, list):  # single yolo model
        exp_name = Path(yolov6_weights).stem
    elif type(yolov6_weights) is list and len(yolov6_weights) == 1:  # single models after --yolo_weights
        exp_name = Path(yolov6_weights[0]).stem
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                cv2.imshow(str(p), cv2.resize(im0, (1280, 720)))
